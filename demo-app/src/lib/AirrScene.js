...
export default class AirrScene extends AirrComponent {
    ...
    prepareViews(views) {
        return views.map(item => {
            ...
            item.props.ref = comp =>
                (this.viewsCompsRefs[item.props.name] = comp);
            return item;
        });
    }
    ...
    componentWillReceiveProps(nextProps) {
        if (!this.viewChangeInProgress) {
            if (this.state.views.length !== nextProps.views.length) {
                ...
                if (this.state.activeViewName !== nextProps.activeViewName) {
                    if (this.state.views.length > nextProps.views.length) {
                        ...
                        this.changeActiveView(nextProps.activeViewName, () => {
                            this.setState({
                                views: this.prepareViews(nextProps.views)
                            });
                ...
                if (!equal) {
                    ...
                    this.state.activeViewName !== nextProps.activeViewName
                ) {
                    ///if only active view has changed

                    if (
                        nextProps.activeViewName !== null &&
        ...
        if (
            this.state.backButtonOnFirstView !== nextProps.backButtonOnFirstView
        ) {
            this.setState({
                backButtonOnFirstView: nextProps.backButtonOnFirstView
            });
        }
    ...
    componentDidMount() {
        if (this.state.navbar && this.state.navbarHeight && this.containerDOM) {
            this.containerDOM.style.height =
                this.containerDOM.parentNode.clientHeight -
        ...
        if (
            ...
            typeof this.viewsCompsRefs[this.state.activeViewName]
    ...
    pushView(config) {
        ...
        config.props.ref = comp =>
            (this.viewsCompsRefs[config.props.name] = comp);
    ...
    openMayer(config) {
        ...
        this.state.mayers.forEach(item => {
            if (item.name === config.name) {
                console.warn(
                    "[Airr] Scene allready has Mayer with this name: " +
                        item.name
                );
                cancel = true;
            }
        ...
        if (mayerConfigIndex >= 0 && this.mayersCompsRefs[name]) {
            this.mayersCompsRefs[name].animateOut(() => {
                ...
                if (mayerConfigIndex >= 0 && this.mayersCompsRefs[name]) {
                    ...
                    const newMayerDefinition = update(this.state.mayers, {
                        $splice: [[mayerConfigIndex, 1]]
                    });
            ...
            this.setState({ GUIDisabled: true, mockTitle: newViewName }, () => {
                ...
                if (this.getViewIndex(newViewName) !== -1) {
                    ...
                    const animEndCallback = () => {
                        ...
                        if (
                            typeof this.props.viewsAnimationEndCallback ===
                            "function"
                        ) {
    ...
    doViewAnimation(newViewName, oldViewName, callback) {
        const newViewDOM =
            this.viewsCompsRefs[newViewName] &&
            this.viewsCompsRefs[newViewName].refs.airrView.refs.dom;
        ...
        if (this.state.animation === "slide" && oldViewName) {
            ...
            if (direction === -1) {
                startProps.webkitTransform =
                    "translate3d(" + -1 * this.refs.dom.clientWidth + "px,0,0)";
            ...
            AirrFX.doTransitionAnimation(
                ...
                () => {
                    ...
                    this.containerDOM.style.webkitTransform =
                        "translate3d(0,0,0)";
            ...
            if (direction === 1) {
                AirrFX.doTransitionAnimation(
                    newViewDOM,
                    {
                        webkitTransform: `translate3d(${this.containerDOM
                            .clientWidth + "px"},0,0)`,
                        transform: `translate3d(${this.containerDOM
                            .clientWidth + "px"},0,0)`,
                        opacity: 0
                    },
                    [
                ...
                if (this.props.stackMode) {
                    AirrFX.doTransitionAnimation(
                        this.viewsCompsRefs[oldViewName].refs.airrView.refs.dom,
                        {
                            webkitTransform: `translate3d(0,0,0)`,
                            transform: `translate3d(0,0,0)`,
                            opacity: 1
                        ...
                        {
                            webkitTransform: `translate3d(0,${this.containerDOM
                                .clientHeight /
                                4 +
                                "px"},0)`,
                            transform: `translate3d(0,${this.containerDOM
                                .clientHeight /
                                4 +
                                "px"},0)`,
                            opacity: 0
    ...
    enableSidepanel() {
        if (this.state.sidepanel) {
            const newPanelDefinition = update(this.state.sidepanel, {
    ...
    render() {
        ...
        if (this.state.sidepanel) {
            sidepanel = React.createElement(
                this.state.sidepanel.type,
                this.state.sidepanel.props
            );
        }
        ...
        if (this.state.navbar) {
            ...
            if (this.state.backButton) {
                const backClassName =
                    "back " +
                    (activeViewIndex < 1 && !this.state.backButtonOnFirstView
                        ? "hidden"
                        : "");
                back = (
                    <div
            ...
            if (this.state.mockTitle) {
                const mockTitleContent =
                    this.state.views[activeViewIndex] &&
                    this.state.views[activeViewIndex].props.title;
                const mockTitleViewIndex = this.getViewIndex(
                    this.state.mockTitle
                );
                mockTitle = this.state.mockTitle ? (
                    <div className="mock-title">
                        <span>{mockTitleContent}</span>
                    </div>
                ) : null;
                title = this.state.views[mockTitleViewIndex]
                    ? this.state.views[mockTitleViewIndex].props.title
                    : "";
            } else {
                title = this.state.views[activeViewIndex]
                    ? this.state.views[activeViewIndex].props.title
                    : "";
            }
            ...
            navbar = (
                ...
                >
                    ...
                    >
                        ...
                        <div
                            className="title"
                            style={{ opacity: this.state.mockTitle ? 0 : 1 }}
                        >
                            <span>{title}</span>
                        </div>
                        {menu}
