...
class AirrSidepanel extends AirrComponent {
    ...
    updateSide() {
        ...
        if (this.side === 'left' || this.side === 'right') {
            ...
            this.size = this.sceneHeight * 1 / 4;
    ...
    enable() {
        if (!this.isEnabled()) {
            this.sceneDOM.addEventListener('touchstart', this.handleTouchStart, Airr.supportsPassive);
            this.enabled = true;
        }
    }
    ...
    handleTouchStart(e) {
        ...
        if (!dragCtnOnTouchPath && ((['left', 'top'].indexOf(this.side) !== -1 && pos < 20)
            ...
            this.sceneDOM.addEventListener('touchmove', this.handleShowTouchMove, Airr.supportsPassive);
            ...
            this.sceneDOM.addEventListener('touchmove', this.handleHideTouchMove, Airr.supportsPassive);
            this.sceneDOM.addEventListener('touchend', this.handleTouchEnd, false);
    ...
    render() {
        ...
        return (
            <div className="sidepanel" ref={dom => this.sidepanelDOM = dom} style={this.sidepanelStyle}>
