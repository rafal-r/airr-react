...
export default class AirrSidepanel extends PureComponent<Props> {
	...
	__bubbleChildTillParent(child: Node, parent: Node, tillElements: Node[]) {
		...
		if (e.path) {
			...
			if (
				...
				this.__bubbleChildTillParent(e.target, this.refDOMDragCtn.current, [
					this.refDOMDragCtn.current.parentNode,
					document.body
				])
			) {
				dragCtnOnTouchPath = true;
	...
	handleShowTouchMove = e => {
		...
		if (newVal !== this.currentVal) {
			...
			this.refDOMBgLayer.current.style.opacity = progress * this.props.bgLayerOpacity;
