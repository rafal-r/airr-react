...
import HelloWorld, { viewName as HelloWorldViewName } from "../views/HelloWorld";
import CommonView, {
...
export default class SimpleScene extends SceneWrapper {
	...
	goToAnotherView = e => {
		...
		this.changeView(firstViewName, {
			children: <h2 style={{ textAlign: "center" }}>We moved back to previous view.</h2>
		});
	};
	...
	viewsConfig = {
		[HelloWorldViewName]: {
			...
			props: {
				...
				children: (
					<button onClick={this[this.props.handleMethod]} style={{ fontSize: "2rem" }}>
						Click me
					</button>
				)
			}
