...
export default function() {
    ...
    const GUIDisabledCoverText = text(
        "GUIDisabledCoverText",
        "",
        GROUP_IDS.GENERAL
    );
    ...
    const properties = {
        ...
        GUIDisabled,
        GUIDisableCover: (
            <div className="gui-mask-cover">{GUIDisabledCoverText}</div>
        ),
        ...
        handleBackButton: getHandleBackButtonValue(
            handleBackButton,
            viewportRef
        ),
        ...
        case GO_BACK:
            return () => {
                const activeViewIndex = viewportRef.current.state.views.findIndex(
                    view =>
                        view.props.name ===
                        viewportRef.current.state.activeViewName
                );
                const nextView =
                    viewportRef.current.state.views[activeViewIndex - 1];
            ...
            return () => {
                ...
                if (nextView) {
                    viewportRef.current
                        ...
                        .then(() =>
                            viewportRef.current.destroyView(lastViewName)
                        );
                }
            };
