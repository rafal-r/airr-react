...
class AirrMayer extends AirrComponent {
    ...
    renderButton(config, index) {
        ...
        return <button key={'btn' + index} className={className} style={config.style || null} onClick={config.handler || null} {...spareAttribs}>{config.content}</button>
    }
...
//        (element, startProps, transitionProps, endProps, preAnimationCallback, endAfter, endCallback) {
        AirrFX.doTransitionAnimation(this.mayerDOM.querySelector('.bg'), {opacity: 0}, ['opacity ' + this.props.animationTime + 'ms ease-out'], {opacity: 1});
        AirrFX.doOverlayInAnimation(this.ctnDOM, this.mayerDOM.clientWidth, this.mayerDOM.clientHeight, this.props.animationTime, this.props.appearFrom);
    }

    animateOut(callback) {
        AirrFX.doTransitionAnimation(this.mayerDOM.querySelector('.bg'), {opacity: 1}, ['opacity ' + this.props.animationTime + 'ms ease-out'], {opacity: 0});
        AirrFX.doOverlayOutAnimation(this.ctnDOM, this.mayerDOM.clientHeight, this.mayerDOM.clientWidth, this.props.animationTime, this.props.leaveTo, callback);
    }
